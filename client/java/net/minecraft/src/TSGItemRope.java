...
public class TSGItemRope extends Item
{
	public TSGItemRope( int iItemID )
	{
		super( iItemID );
		SetBuoyant();
	...
	public boolean
	isValidAnchor(World world, int i, int j, int k, int id){
		return ( id == FCBetterThanWolves.fcAnchor.blockID && ((FCBlockAnchor)FCBetterThanWolves.fcAnchor).GetFacing( world, i, j, k ) != 1 ) ||
			id == FCBetterThanWolves.fcRopeBlock.blockID || id == TSGTheifarrows.taBlockAnchor.blockID;
	}
	@Override
	public boolean onItemUse( ItemStack itemStack, EntityPlayer player, World world, int i, int j, int k, int iFacing, float fClickX, float fClickY, float fClickZ )	
	{
		...
		if (isValidAnchor(world, i,j,k, iTargetBlockID))
		{
